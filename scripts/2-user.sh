...
sed -n '/'$INSTALL_TYPE'/q;p' ~/nabucodonosor/pkg-files/${DESKTOP_ENV}.txt | while read line; do
...
if [[ ! $AUR_HELPER == none ]]; then
	...
	sed -n '/'$INSTALL_TYPE'/q;p' ~/nabucodonosor/pkg-files/aur-pkgs.txt | while read line; do
		if [[ ${line} == '--END OF MINIMAL INSTALL--' ]]; then
			# If selected installation type is FULL, skip the --END OF THE MINIMAL INSTALLATION-- line
			continue
		fi
